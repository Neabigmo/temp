...
Springer Nature officially recommends its journal-article LaTeX template for Springer Nature journals, including Springer titles such as PTRF. Use `main_ptrf.tex` with the bundled official `sn-jnl.cls` and `sn-mathphys-num.bst` files in this flat package. The portable article-class entry point is intentionally excluded: this directory is the single official working version.
