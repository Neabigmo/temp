...
We prove Theorem~\ref{thm:exact-psd} and then apply it to the ordinary sample variance. Write $A=B^{\mathsf T}B$, where $B\in\R^{r\times d}$ has full row rank, and let $b_i$ be the $i$th column of $B$. If $V$ is uniform on $S^{r-1}$, write
\[
\Psi_r(z):=\E_V e^{zV_1}.
\]
