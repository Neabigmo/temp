...
\qquad |s|\le\rho_0,
\]
where $F(s)=M_Y(s)-e^{s^2}$ and $Y=X-X'$. Apply the subharmonic two-constants theorem to $u(z)=\log|F(z)|$ on each half-ellipse. If $\omega(z)$ denotes the harmonic measure of the real diameter, then
\[
...
\]
For $N=B\sqrt m>\sqrt2$, Lemma~\ref{lem:moment-transfer} and Markov's inequality give
\[
\Pp(|X|>N)
\le \Pp(|X-a|>N-\sqrt2)
\le4\frac{\E|Y|^{2m}}{(N-\sqrt2)^{2m}}
...
\le\frac{|u|^{2m}}{(2m)!}.
\]
The comparison has three contributions:
\[
\begin{aligned}
|\varphi_Y(t)-e^{-t^2}|
