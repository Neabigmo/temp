...
\]
For $r\ge1$, let $V$ be uniform on $S^{r-1}$ and define
\[
\Psi_r(z):=\E_V e^{zV_1},
\qquad
\mathfrak D_{r,\tau}(Q):=
...
\]
This spherical Laplace discrepancy is the natural observable for a squared radial projection; it is not a generic probability distance.
...
We now make the discrepancy-to-defect transfer explicit. Let $r=n-1$, let $P=I_n-n^{-1}\mathbf1\mathbf1^{\mathsf T}$, and choose $B\in\R^{r\times n}$ with $B^{\mathsf T}B=P$ and $BB^{\mathsf T}=I_r$. If $V$ is uniform on $S^{r-1}$, then every column $b_i$ of $B$ has norm
\[
...
\end{equation}
This is the point at which the spherical observable enters the one-dimensional reflection theory.
...
\]
because $M_X(s)\ge1$ and $|x|e^{\tau|x|}\le(e\tau)^{-1}e^{2\tau|x|}$. If $M_*:=\sup_{|s|\le\rho'}D_X(s)$, two disjoint Lipschitz tents centered at points where the maximum is attained give
\[
2m_h I_{L_h,h}(M_*)
...
Applying \eqref{eq:pointwise-D} to the sample-variance hypothesis and then Theorem~\ref{thm:sharp-upper} proves Theorem~\ref{thm:sample-var}. In the exact case, Corollary~\ref{cor:exact-sample-var} gives Gaussianity under the same dominance condition. Thus the discrepancy-to-defect argument is a quantitative extension of the exact spherical-Jensen mechanism.

Two questions remain open. Can fixed exponential integrability be replaced by a sharp tail or moment hypothesis without changing the reflection modulus? Under which additional uniform-integrability hypothesis can a standard distributional distance replace $\mathfrak D_{r,\tau}$ while retaining the same interior pointwise transfer? A further direction is to extend the factor-recovery and optimality theory from the i.i.d. sample-variance projection to general positive-semidefinite quadratic forms with non-identically distributed coordinates.
