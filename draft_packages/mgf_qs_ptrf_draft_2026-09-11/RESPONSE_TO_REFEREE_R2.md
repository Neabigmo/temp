...
`sections/introduction.tex`, Section 1, the Cramér/Marcinkiewicz positioning paragraphs. The bibliography retains Cramér, Sapogov, and Bobkov--Chistyakov--Götze and adds the standard references used by the proof.
...
`sections/sharp_upper.tex`, Lemma `two-constants` and Lemma `zero-free`; `appendices/parameter_selection.tex`, the non-circular choice chain and the definition of $\gamma_*$.
...
`sections/sharp_upper.tex`, Section 3, Lemma `factor-gaussian`; `references.bib` and `references_manual.tex`, entries `Duren1983` and `Esseen1945`.
...
`sections/sharpness.tex`, the definition of $J_m$ and the display preceding the reflected-defect identity.
...
`sections/introduction.tex`, Theorem 1.3; `sections/sharpness.tex`, the end of the Laguerre proof; repository-wide exact search for the prohibited monotone-decrease notation.
...
`sections/sharpness.tex`, the exact MGF and half-axis derivations; `appendices/laguerre_details.tex`, the complete four-region proof; `references.bib` and `references_manual.tex`, the IRT citation.
...
`sections/sample_variance.tex`, the discrepancy definition, theorem title, pointwise transfer, and final rate substitution.
